...
interface SyncProxyOptions {
   ...
   * currently disabled in PingClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync PingClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because PingClaw proxy is disabled and preserve mode is enabled');
      return;
    }
