...
import { getPingClawProviderStore } from '../services/providers/store-instance';
import {
...
export async function storeApiKey(providerId: string, apiKey: string): Promise<boolean> {
  try {
    ...
    const s = await getPingClawProviderStore();
 ...
 * Also synchronizes PingClaw local provider list with OpenClaw's actual config.
 */
...
> {
  ...
  for (const provider of providers) {
    ...
    // we should remove it from PingClaw UI to stay consistent.
