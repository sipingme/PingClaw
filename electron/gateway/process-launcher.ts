...
(function () {
  ...
  if (globalThis.__pingclawFetchPatched) return;
  globalThis.__pingclawFetchPatched = true;

  globalThis.fetch = function pingclawFetch(input, init) {
    var url =
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__pingclawPatched) {
        cp.__pingclawPatched = true;
...
export async function launchGatewayProcess(options: {
  ...
  // which causes:
  //   - cross-machine name collisions when multiple OpenClaw/PingClaw peers
  //     share a LAN (each falls back to "<name> (OpenClaw) (2)")
  //   - self-collisions on multi-homed hosts (Wi-Fi + Tailscale + utun ...)
  //   - "ghost" record collisions after an unclean PingClaw exit, because
  //     SIGKILL prevents ciao from emitting the mDNS goodbye record.
  //
  // PingClaw has no UI for LAN gateway discovery today, so the advertiser is
