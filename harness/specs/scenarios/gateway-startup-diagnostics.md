...
Use this spec when PingClaw shows the Gateway as starting/running but UI data does not refresh, Dreams cannot load, or Gateway RPC calls time out after a restart.

PingClaw should prefer OpenClaw-native signals over stderr string matching:
...
- `gateway.ready`, `health`, and `presence` events should update PingClaw's main-process capability cache.
...
- **Handshake ready** only means PingClaw connected to the Gateway socket.
...
2. Read recent PingClaw logs:

```bash
tail -n 160 "$HOME/Library/Application Support/pingclaw/logs/pingclaw-$(date +%F).log"
```
...
```bash
pnpm exec openclaw gateway call system-presence >/tmp/pingclaw-system-presence.json
pnpm exec openclaw gateway call health --params '{"probe":false}' >/tmp/pingclaw-health.json
pnpm exec openclaw gateway call status >/tmp/pingclaw-status.json
pnpm exec openclaw gateway call channels.status --params '{"probe":false}' >/tmp/pingclaw-channels-status.json
pnpm exec openclaw gateway call doctor.memory.status >/tmp/pingclaw-memory-status.json
pnpm exec openclaw gateway call doctor.memory.dreamDiary >/tmp/pingclaw-dream-diary.json
```
...
```bash
sample <gateway-pid> 3 -mayDie >/tmp/pingclaw-gateway.sample.txt
```
...
- Where the artifact will be written, usually `/tmp/pingclaw-gateway.sample.txt`.
- What will be inspected and what will not be shared verbatim.

Do not proceed without explicit user agreement when:

- Sampling a process that is not clearly the PingClaw-owned Gateway child.
...
- Whether the finding points to PingClaw-owned prelaunch cleanup, OpenClaw runtime startup cost, active user work, or inconclusive data.
...
- Explain to users that restart cost is dominated by active Gateway work, not by PingClaw UI rendering.
...
- Logs no longer repeat stale runtime cache or escaped managed-skill symlink warnings for entries PingClaw can safely clean.
...
- Separate PingClaw-owned cleanup issues from OpenClaw runtime initialization cost.
