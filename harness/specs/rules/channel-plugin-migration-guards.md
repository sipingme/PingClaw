...
When channel plugin ownership changes between bundled OpenClaw extensions and external `~/.openclaw/extensions/*` installs, PingClaw must normalize configuration to one active plugin identity per channel.
...
- when the canonical Feishu plugin is external, PingClaw must explicitly disable the bundled `feishu` plugin instead of only removing allowlist entries
