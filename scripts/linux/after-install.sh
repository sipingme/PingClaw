...
# Post-installation script for PingClaw on Linux
...
# Create symbolic link for PingClaw app binary
if [ -x /opt/PingClaw/pingclaw ]; then
    ln -sf /opt/PingClaw/pingclaw /usr/local/bin/pingclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/PingClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/PingClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/PingClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/PingClaw/resources/apparmor-profile'
...
echo "PingClaw has been installed successfully."
